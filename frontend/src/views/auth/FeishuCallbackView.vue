...
async function finalizePendingAccountResponse(completion: FeishuPendingActionResponse) {
  ...
  if (completion.step === 'email_completion' || (completion as Record<string, unknown>)['requires_email_completion'] === true) {
    await router.replace('/auth/feishu/email-completion?redirect=' + encodeURIComponent(redirect))
    return
  }
...
onMounted(async () => {
  ...
  try {
    ...
    const completionData = completion as FeishuPendingActionResponse
    ...
    if (completionData.step === 'email_completion' || (completionData as unknown as Record<string, unknown>)['requires_email_completion'] === true) {
      ...
      await router.replace('/auth/feishu/email-completion?redirect=' + encodeURIComponent(completionRedirect))
      return
    }
