...
func TestBackendModeAuthGuard(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "enabled_allows_feishu_complete_registration",
			enabled:    "true",
			path:       "/api/v1/auth/oauth/feishu/complete-registration",
			wantStatus: http.StatusOK,
		},
		{
			name:       "enabled_allows_feishu_create_account",
			enabled:    "true",
			path:       "/api/v1/auth/oauth/feishu/create-account",
			wantStatus: http.StatusOK,
		},
		{
			name:       "enabled_allows_feishu_bind_login",
			enabled:    "true",
			path:       "/api/v1/auth/oauth/feishu/bind-login",
			wantStatus: http.StatusOK,
		},
		{
